...
def main(args: argparse.Namespace, config, rank):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch+1) % 10 == 0:
            ...
            print("curr_acc={:.2f}, best_acc = {:.2f}".format(curr_acc, best_acc))
            config["out_file"].write("curr_acc={:.2f}, best_acc = {:.2f}".format(curr_acc, best_acc) + '\n')
            config["out_file"].flush()
        if epoch > 69:
            domain_discrenpancy = discriminator_capacity(train_target_loader, classifier, feat_adnet, device)
            lfs.update_lfs_max(domain_discrenpancy)
