...
def main(args: argparse.Namespace, config, rank):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < (args.start_lfs+1) and (epoch+1) % 5 ==0:
        ...
        if epoch > args.start_lfs:
            if args.evaluation == 'mmd':
                mmd = get_mmd(train_source_iter, train_target_iter, classifier, device, mmd_loss, args)
                mmd = mmd_ema.update(mmd)
                lfs.update_lfs(mmd)
            elif args.evaluation == 'entropy':
                domain_discrenpancy = discriminator_capacity(train_target_loader, classifier, feat_adnet, device)
...
def train(train_source_iter: ContinuousDataloader, train_target_iter: ContinuousDataloader, model, optimizer: SGD, feat_adnet, pred_adnet, lfs, lr_sheduler: LrScheduler, epoch: int, args: argparse.Namespace, device):
    ...
    for i in range(args.iters_per_epoch):
        ...
        if args.pred_process:
            rho_pad = args.rho_pad + rho * (1 - args.rho_pad) 
        else:
            rho_pad = 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--evaluation', type=str, default='mmd', choices=['mmd', 'entropy'])
    ...
    parser.add_argument('--rho_pad', type=float, default=0.1)
    ...
    parser.add_argument('--start_lfs', default=29, type=int, metavar='N', help='number of total epochs to run')
