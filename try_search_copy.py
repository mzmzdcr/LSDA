...
from common.tools import get_center_feature
warnings.filterwarnings("ignore")
...
def main(args: argparse.Namespace, config, rank):
    ...
    mis_cls = torch.zeros(args.num_classes).bool()

    for epoch in range(args.epochs):
        if epoch > 30:
            mis_cls = get_mis_cls(train_source_loader, train_target_loader, classifier, device, epoch, args.num_classes)
            lfs.set_alpha_parameters(epoch, device)
            lfs.set_gamma_parameters_copy(mis_cls, device)
...
def get_mis_cls(src_loader, tar_loader, model, device, e, cls_num):
    src_f = get_center_feature(src_loader, model, device, cls_num)
    tar_f = get_center_feature(tar_loader, model, device, cls_num)
    e += 1
    thresh = 0.5
    mis_cls = domains_cosine(src_f, tar_f, thresh)
    return mis_cls


def domains_cosine(src, tar, thresh):
    src = F.normalize(src, p=2, dim=1)
    tar = F.normalize(tar, p=2, dim=1)
    cos_sim = torch.matmul(tar, src.t())
    cc = torch.diag(cos_sim)
    cc_diag = torch.diag_embed(cc)
    cos_sim = cos_sim - cc_diag
    a = (cos_sim > thresh)
    b = torch.sum(a, dim=1)
    mis_cls = (b!=0)
    return mis_cls
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dset', type=str, default='home', choices=['office', 'home', 'domainnet', 'birds'], help="The dataset used")
    parser.add_argument('--task', type=str, default='A2C', help="task source 2 target")

    parser.add_argument('--save_path', type=str, default='/remote-home/source/47/meizhen/code/work_1/lsda_search_0302/log/search/cdan_Dp_ort/home', help="output directory of logs")
    parser.add_argument('--data_path', type=str, default='/remote-home/source/47/meizhen/dataset/office_home/source')
    ...
    parser.add_argument('--mmd_iter', default=500, type=int, help='Number of iterations per epoch')
    ...
    parser.add_argument('--iters-per-epoch', default=100, type=int, help='Number of iterations per epoch')
