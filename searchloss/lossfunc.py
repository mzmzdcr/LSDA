...
class LossFuncSearch(object):
    ...
    def set_gamma_parameters_copy(self, mis_c, device=None):
        self.gamma = self.gamma_agent.sample_subfunction(mis_c)
        g = mis_c.float() * math.exp(1) + 1.0
        g = g.to(device)
    ...
    def update(self, reward):
        ...
        print('discrepancy', test_acc_tensor)
        best_test_acc_rank = torch.argmin(test_acc_tensor)
        best_test_acc_rank = torch.tensor(np.random.randint(0, len(test_acc_tensor)))
